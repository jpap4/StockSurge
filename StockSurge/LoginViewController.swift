...
extension LoginViewController: FUIAuthDelegate {
    ...
    func authPickerViewController(forAuthUI authUI: FUIAuth) -> FUIAuthPickerViewController {
        let marginInsets: CGFloat = 30 // amount to indent UIImageView on each side
        ...
        let height = UIScreen.main.bounds.height - (topSafeArea) - (marginInsets)
        
        let logoFrame = CGRect(x: x, y: y, width: width, height: height)
        
        // Create the UIImageView using the frame created above & add the "logo" image
        let logoImageView = UIImageView(frame: logoFrame)
        logoImageView.image = UIImage(named: "StockSurgeLogo")
